...
- **이웃을 보고 그리는 타일**(tileart.js `BODY_ONLY` · `CONN` · `drawConn`): 윗면을 제 그림에 그린
  ...
  벗은 꼭대기는 둥글게) · 야자 줄기(`_palmTile` — **칸 격자를 따른다**: 칸 가운데 곧은 줄기, 이음줄은 두 칸을 잇는 직각 가로 띠) ·
  야자 잎(`_palmLeafTile` — 잎 칸을 꽉 채우고 트인 가장자리만 깎는다, 잎결은 닻 `_palmAnchor` 에서 처지는 사선)도 세계 좌표로 칠해
  칸 경계에서 결이 이어진다. 칸 밖으로 그리는 자유 곡선은 쓰지 않는다. 아틀라스는 **칸마다 잘라서** 그린다
