...
- **이웃을 보고 그리는 타일**(tileart.js `BODY_ONLY` · `CONN` · `drawConn`): 윗면을 제 그림에 그린
  ...
  벗은 꼭대기는 둥글게) · 야자 줄기(`_palmLine` 이 한 그루의 가운데선을 따라 부드럽게 굽힌다) · 야자 잎(`_palmCrownAt` — 줄기
  꼭대기 위 잎 칸을 닻으로 잎갓 한 장을 그려 칸마다 잘라 씀)도 세계 좌표로 칠해 칸 경계에서 결이 이어진다. 아틀라스는 **칸마다 잘라서** 그린다
