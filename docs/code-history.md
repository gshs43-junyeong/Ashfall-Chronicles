...
> 뭔가 낀 것처럼만 보인다).
> → 그랬더니 문을 열면 문이 **사라져** 문이 없는 것처럼 보였다. 이제 흐리지 않고, 다 열린
> 문짝은 문틀 폭의 38%(최소 8px)로 문틀 끝에 또렷이 선다 — 실오라기가 아니라 옆에서 본 문짝이다.
> 움직임: 예전엔 0.14초에 폭이 일정하게 줄어 '미끄러지는' 것처럼 보였다. 경첩을 도는 판처럼 폭을 cos(각)으로
> 줄이고(처음 천천히 · 끝에서 빨리) 0.3초에 열리며, 돌아간 만큼 조금 어두워진다.
> 열린 문짝을 문틀 밖으로 조금(폭의 12%) 젖혀 그렸더니 옆 칸을 침범해 보였다 — 이제 제 칸 안에서만 그린다.
> 손잡이는 obj/door.png 것을 그대로 쓴다 — 경첩 반대쪽이라 젖혀질수록 먼저 말려 든다.
>
> → 사용자 요청으로 다시 뒤집었다(2026-09-25): **닫힌 문 = 옆모습**(경첩 쪽 가장자리의 얇은 판, 폭 32%·최소 6px),
> **열린 문 = 칸을 채운 앞면**. 이번엔 판정이 그림을 따른다 — 닫힌 문은 그 얇은 판(`doorEdge`)만 막아서 문 칸의 나머지
> 자리에 설 수 있고, 닫을 때 막히는 것도 그 판과 겹칠 때뿐이다. 폭은 sin(각)으로 넓어진다(옆모습에서 앞면으로 도는 판).
...
<a id="h140"></a>
### h140 — `sprites.js` · `drawObj` / `entity.js` · `equipFrom` / `world.js` · 무너지는 발판

> `window.G` 는 한 번도 없었다 — G 는 `const` 라서 전역 객체에 실리지 않는다. 그래서 `window.G` 를 보던
> 세 곳은 v1.1.0 까지 **늘 꺼져 있었다**: 두 장짜리 시설물 그림은 첫 장에 멈춰 있었고(`drawObj` 의 t 가 늘 0),
> 레벨이 모자란 장비를 끼우면 알림 없이 안 끼워졌고(`equipFrom`), 무너지는 발판은 먼지 없이 사라졌다.
> 모듈로 옮기면서(P2) `window.G` 가 디버그 창구로 생겼다 — 그대로 두면 세 곳이 갑자기 켜진다.
> 엔진화 단계는 동작을 바꾸지 않으므로 **꺼진 그대로 적어 두었다.** 살리는 것은 따로 정해서 한다
> (G.time 을 ctx 로 읽고 · G.toast · G.parts 에 Part 를 넣으면 된다 — 스크린샷 기준 그림이 바뀔 수 있다).
