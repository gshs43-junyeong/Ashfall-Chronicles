...
const T = {
  ...
  M_PRESSOR: 127, M_DESAL: 128, M_BELT_F: 129, M_BATTERY_HI: 130,
  /* --- v1.1 세션 3: 새 함정. 피해보다 **숨**을 빼앗는다 --- */
  BRINEVENT: 131,
  /* --- v1.1: 촉발 지뢰. 밟으면 터진다 --- */
  TRIPMINE: 132,
  /* --- v1.1 세션 3: 바다·해변 장식 --- */
  KELPPLANT: 133, SEASHELL: 134,
  /* --- v1.1 H: 화약 원료. 세션 3(빙하·해저)에서만 나온다 --- */
  SULFUR: 135, ROOMAIR: 136, PALMWOOD: 137, PALMLEAF: 138, COCONUT: 139, GLACIUM: 140, TIDESTONE: 141
};
...
const ENEMIES = {
  ...
  mine_horror:  { n: '갱도의 것', hp: 1900, dmg: 52, def: 16, spd: 96, ai: 'b_slime', w: 52, h: 44, c: '#6a5a4a', xp: 1500, gold: 620, boss: 1,
                 drops: [['rust_gear', 1, 2, 3], ['iron_ore', 1, 20, 30], ['lost_lamp', 1, 1, 2]] },
  /* rank 2 */
  ice_warden:   { n: '얼음 감시자', hp: 2800, dmg: 75, def: 24, spd: 74, ai: 'b_witch', w: 40, h: 54, c: '#9fd8f0', xp: 2300, gold: 950, boss: 1,
                 drops: [['frozen_core', 1, 2, 3], ['frost_core', 1, 8, 14], ['ice_shard', 1, 20, 30]] },
  /* rank 3 */
  vine_lord:    { n: '덩굴 군주', hp: 4100, dmg: 105, def: 34, spd: 80, ai: 'b_bone', w: 44, h: 58, c: '#3f7a34', xp: 3400, gold: 1500, boss: 1,
                 drops: [['heartwood', 1, 2, 3], ['vine_coil', 1, 12, 20], ['orchid', 1, 8, 14]] },
  /* rank 4 — 함정이 가장 촘촘한 유적의 주인 */
  sand_guardian:{ n: '모래 파수꾼', hp: 5600, dmg: 138, def: 46, spd: 66, ai: 'b_bone', w: 46, h: 60, c: '#d8b878', xp: 4800, gold: 2100, boss: 1,
                 drops: [['sun_disc', 1, 2, 3], ['gold_ore', 1, 15, 25], ['venom_sting', 1, 6, 10]] },
  /* rank 5 — 입구가 없는 굴. 도망칠 길이 없다 */
  spore_queen:  { n: '포자 여왕', hp: 7200, dmg: 174, def: 42, spd: 92, ai: 'b_heart', w: 46, h: 46, c: '#6fe0c0', xp: 6400, gold: 2800, boss: 1,
                 drops: [['queen_spore', 1, 2, 3], ['spore_sac', 1, 12, 20], ['glowcap', 1, 15, 25]] },
  /* rank 6 — 동쪽 끝, 가장 깊은 곳 */
  blight_maw:   { n: '부패한 아가리', hp: 9400, dmg: 220, def: 58, spd: 88, ai: 'b_heart', w: 50, h: 50, c: '#7a3f9c', xp: 9000, gold: 4000, boss: 1,
  ...
  void_king:   { n: '공허의 왕', hp: 12000, dmg: 94, def: 32, spd: 110, ai: 'b_void', w: 74, h: 92, c: '#5e3fa8', xp: 9000, gold: 5000, boss: 1,
  ...
  first_keeper: { n: '최초의 파수꾼', hp: 20000, dmg: 155, def: 52, spd: 96, ai: 'b_keeper', w: 70, h: 88, c: '#c8b98a', xp: 40000, gold: 20000, boss: 1,
                 minion: 'ruin_guard',
                 drops: [['aether_shard', 1, 40, 60], ['ruin_brick', 1, 40, 70], ['charm_rune', 1, 1, 1], ['star_heart', 1, 2, 2]] },

  /* 종장 — 별이 도망쳐 온 그것 */
  pursuer:      { n: '별을 쫓아온 것', hp: 42000, dmg: 224, def: 68, spd: 128, ai: 'b_pursuer', w: 96, h: 104, c: '#2a2036', xp: 120000, gold: 60000, boss: 1,
                 minion: 'wraith', aggro: 4000,
                 drops: [['void_frag', 1, 60, 90], ['star_heart', 1, 3, 3], ['charm_dawn', 1, 1, 1], ['scythe_void', 1, 1, 1]] },

  /* 7단계 보스 */
  proliferator: { n: '증식체', hp: 46000, dmg: 247, def: 62, spd: 112, ai: 'b_slime', w: 88, h: 72, c: '#9a8a76', boss: 1,
                 xp: 150000, gold: 70000, minion: 'splitter',
                 drops: [['core_shard', 1, 40, 60], ['machine_frame', 1, 6, 10], ['power_core', 1, 25, 40]] },
  hepha:        { n: '헤파 · 최초의 기계', hp: 72000, dmg: 326, def: 80, spd: 120, ai: 'b_keeper', w: 92, h: 110, c: '#c8a05a', boss: 1,
                 xp: 400000, gold: 180000, minion: 'coreling', aggro: 4000,
                 drops: [['hepha_heart', 1, 1, 1], ['core_shard', 1, 60, 90], ['aether_shard', 1, 30, 45]] },

  /* 세션 2 — 지하 공창의 관리자 */
  overseer:     { n: '공창의 관리자', hp: 30000, dmg: 210, def: 60, spd: 104, ai: 'b_keeper', w: 74, h: 92, c: '#8a8a96', xp: 90000, gold: 44000, boss: 1,
                 minion: 'riveter',
  ...
  archetype:    { n: '원형 · 첫 번째 설계', hp: 105000, dmg: 385, def: 92, spd: 126, ai: 'b_keeper', w: 96, h: 116, c: '#e8dcc0', boss: 1,
  ...
  restorer:     { n: '환원기 · 되돌리려는 것', hp: 320000, dmg: 774, def: 130, spd: 132, ai: 'b_restorer', w: 118, h: 132, c: '#a8c8e8', boss: 1,
  ...
  shaft_maw:    { n: '갱을 메운 것', hp: 88000, dmg: 516, def: 112, spd: 74, ai: 'b_heart', w: 92, h: 84, c: '#3a342c', boss: 1,
...
const RUIN_SPEC = [
  {
    id: 'ice', n: '얼음 던전', x: 300 + SHIFT, y: 150, w: 74, h: 44,
    wall: T.ICEBRICK, floor: T.ICE, bg: 5, torch: T.TORCH,
    traps: ['dart', 'crumble'], boss: 'ice_warden',
    mobs: ['frostling', 'icewolf'],
    rank: 2, tier: 3, trapRate: 0.68, spikeRate: 0.26, chestRate: 0.34, mobMul: 1.0
  },
  {
    id: 'pyramid', n: '피라미드', x: 2180 + SHIFT, y: 96, w: 66, h: 52,
    ...
    rank: 4, tier: 4, trapRate: 0.96, spikeRate: 0.46, chestRate: 0.42, mobMul: 1.35
  },
  {
    id: 'mine', n: '버려진 광산', x: 820 + SHIFT, y: 168, w: 64, h: 34,
    ...
    rank: 1, tier: 2, trapRate: 0.52, spikeRate: 0.16, chestRate: 0.30, mobMul: 0.85
  },
  {
    id: 'blight', n: '부패한 둥지', x: 4020 + SHIFT, y: 196, w: 84, h: 52,
    ...
    rank: 6, tier: 6, trapRate: 1.0, spikeRate: 0.58, chestRate: 0.52, mobMul: 1.85
  },
  {
    id: 'spore', n: '포자 굴', x: 3620 + SHIFT, y: 176, w: 68, h: 42,
    ...
    rank: 5, tier: 5, trapRate: 0.74, spikeRate: 0.30, chestRate: 0.48, mobMul: 1.6
  }
];
...
/* 입구 통로 자체의 성격(entryKind) — arch(바깥 생김새)와는 별개 축이다.
   ...
   slopeR/slopeL(비탈형): 한쪽으로만 밀리며 내려간다. 미로형과 달리 좌우로 번갈아
                        꺾이지 않아 "오른쪽(왼쪽) 아래로 파고든다"가 그대로 느껴진다
   wild(불규칙형):      다리 길이도 방향도 세로/가로도 전부 굴린다. 막다른 굴까지 판다
...
/* --- 세션 3: 가라앉은 유적 (비밀) ---
   ...
   해저 바닥에 갈라진 틈 하나로만 들어간다(arch 'sunken'). 틈 입구는 바닷물에 잠겨
   있어서 헤엄쳐 들어가야 하고, 거기서 400칸을 더 내려간다 — 산소가 곧 입장료다.
...
RUIN_SPEC.push({
  ...
  traps: ['brine', 'dart', 'crumble', 'brine'], boss: 'drowned_keeper',
  mobs: ['ruin_guard', 'archivist', 'lantern'],
  rank: 7, tier: 5, trapRate: 1.0, spikeRate: 0.5, chestRate: 0.5, mobMul: 1.25,
  arch: 'sunken', rooms: 26, maze: 1, entryKind: 'maze'
});

/* 유적마다 **들어가는 맛이 달라야 한다.** 예전에는 통로 종류가 셋뿐인 데다 심기는 함정이
   전부 화살 구멍이라, 어느 유적을 들어가든 똑같았다. 통로 종류와 함정 목록을 함께 갈랐다.
   traps 목록은 방 안 함정에도 그대로 쓰인다(putTileTrap). */
RUIN_SPEC[0].arch = 'surface'; RUIN_SPEC[0].rooms = 12; RUIN_SPEC[0].entryKind = 'collapse';     // ice   rank 2 — 얼음이 꺼진다
RUIN_SPEC[0].traps = ['crumble', 'dart', 'spike', 'crumble'];
RUIN_SPEC[1].arch = 'surface'; RUIN_SPEC[1].rooms = 16; RUIN_SPEC[1].entryKind = 'slopeR';         // pyramid rank 4 — 오른쪽 아래로 파고드는 경사로
RUIN_SPEC[1].traps = ['dart', 'vent', 'mine', 'crumble'];
RUIN_SPEC[2].arch = 'gated';   RUIN_SPEC[2].rooms = 10; RUIN_SPEC[2].entryKind = 'wild';     // mine  rank 1 — 갱이 무너져 길이 엉망이 됐다
RUIN_SPEC[2].traps = ['crumble', 'dart', 'mine'];
RUIN_SPEC[3].arch = 'buried';  RUIN_SPEC[3].rooms = 18;                                          // blight rank 6 — 가장 크다, 입구 없음
RUIN_SPEC[3].traps = ['vent', 'mine', 'spike', 'dart', 'crumble'];
...
const ACHIEVEMENTS = [
  ...
  { id: 'a_session2', cat: 'story', t: 'mid', i: '🧱', n: '벽 너머', d: '세션 2에 들어선다.',
    check: g => g.chapter >= SESSION_START[1] },
  { id: 'a_session3', cat: 'story', t: 'mid', i: '🌊', n: '가라앉은 쪽', d: '세션 3에 들어선다.',
    check: g => g.chapter >= SESSION_START[2] },
...
/* 세션이 시작하는 챕터. game.js가 `chapter >= 9` 로 세션 2를 하드코딩하고 있었는데,
   세션 3이 붙으면 그 줄을 또 고쳐야 해서 표로 뽑았다. 세션 3 챕터가 정해지면 15를
   실제 값으로 맞추면 된다. */
const SESSION_START = [0, 9, 15];
function sessionOf(chapter) {
  let s = 1;
  for (let i = 0; i < SESSION_START.length; i++) if (chapter >= SESSION_START[i]) s = i + 1;
  return s;
}
...
const CHAPTERS = [
  ...
  /* ================= 세 션 3 =================
     ...
     여기서 매듭지으면 안 된다. 세션 경계는 data.js의 SESSION_START가 잡는다. */
  {
