...
const G = {
  ...
  biomeAir(camX, camY) {
    ...
    /* ★ 색도 섞는다. 예전에는 `k > 0.25 ? B.c : A.c` 였다 — 투명도는 부드럽게 섞이는데
       색만 k 가 0.25 를 넘는 **한 프레임에 통째로** 갈렸다. 그래서 경계를 걸어 넘을 때
       공기색이 뚝 튀었다(잿빛 숲의 회녹색에서 정글의 초록으로 한 번에).
       두 색을 채널별로 섞으면 넘어가는 내내 이어진다. */
    /* k 는 0(한복판)~0.5(경계 한가운데)다 — 경계에서 딱 반반이라는 뜻이므로 그대로
       쓴다. 두 배로 키우면 경계에서 100% 상대 색이 되어, 넘어서는 순간 A·B 의 자리가
       뒤바뀌며 또 튄다(처음 고칠 때 그렇게 해서 75단계나 뛰었다).
       진하기(a)가 이미 같은 규칙으로 섞이고 있었으므로 색만 맞춰 준 셈이다. */
