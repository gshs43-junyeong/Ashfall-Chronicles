...
/* ================= 세이브 무결성 =================
   **먼저 분명히 해 둘 것**: 브라우저 게임에서 저장을 "고칠 수 없게" 만드는 것은
   불가능하다. 검사하는 코드가 같은 기계 안에 그대로 있으므로, 마음먹은 사람은
   서명 함수를 읽고 값을 다시 계산해 넣으면 된다. 콘솔에서 `G.player.gold`를 직접
   바꾸는 것은 아예 저장을 거치지도 않는다.
   여기서 하는 일은 **문턱을 올리는 것**이다 — 개발자 도구를 열고 숫자만 고쳐 쓰는
   가장 흔한 방식은 막힌다. 그 이상은 못 막고, 못 막는다고 적어 둔다.

   방식: 저장한 글자열의 서명을 **따로 둔 칸**에 적어 두고, 열 때 다시 계산해 맞춰 본다.
   서명을 세이브 안에 넣지 않은 이유는 두 가지다 —
     ① JSON 안에 넣으면 키 순서에 따라 다시 만든 글자열이 달라져 헛되이 어긋난다
     ② 문자열을 감싸 넣으면 이스케이프가 겹쳐 용량이 크게 분다(이미 용량 한계가 빠듯하다)
   같은 이유로 **암호화나 base64도 쓰지 않는다** — 세이브가 수 MB라 base64는 1/3을
   더 불려 저장 실패를 부른다. 읽히는 것 자체는 막지 않고, 고친 것을 잡아내기만 한다. */
const SAVE_SALT = 'ashfall-1.1-seal';
...
/** 열어도 되는 기록인가. 옛 기록(sealed 없음)은 그냥 통과시킨다 — 판을 올렸다고
    남의 진행을 못 열게 만들 수는 없다. 다음 저장 때 저절로 봉인된다. */
function saveSealOk(slot, raw, d) {
  if (!d || !d.sealed) return true;                 // 봉인 이전 기록
  let sig = null;
...
const G = {
  ...
  _newGame(seed, name) {
    ...
    if (qs.get('debug') === 'village') {
      ...
      const sess = clamp(+qs.get('sess') || 2, 1, SESSION_START.length);
      this.chapter = qs.get('ch') !== null ? +qs.get('ch') : SESSION_START[sess - 1];
    ...
    if (qs.get('debug') === 'sea') {
      ...
      this.chapter = qs.get('ch') !== null ? +qs.get('ch') : SESSION_START[2];
  ...
  tickTileTraps() {
    ...
    for (let y = y0; y <= y1; y++) {
      for (let x = x0; x <= x1; x++) {
        ...
        if (!def.tdart && !def.tvent && !def.tbrine && !def.tmine) continue;
  ...
  tryFish() {
    ...
    /* 입질까지 걸리는 시간 — 예전엔 1.2~2.8초라 던지자마자 끝나서 "낚시"라는 느낌이
       없었다. 10~60초 사이로 늘린다. 좋은 낚싯대(fishWait가 작다)는 그만큼 덜 기다린다.
       배율을 곱한 뒤에도 10초 밑으로는 안 내려가게 잘라 둔다. */
    const wait = clamp(this.rng.range(13, 58) * (rod.fishWait === undefined ? 1 : rod.fishWait), 10, 60);
  ...
  updateFishing(dt) {
    ...
    if (!f.biting) {
      f.t -= dt;
      if (f.t <= 0) {
        f.biting = true; f.bite = 1.6;      // 챔질할 수 있는 창
  ...
  talkTo(id) {
    ...
    /* 대사는 챕터 번호로 고른다. 세션 3에서 처음 만나는 NPC는 대사가 15장부터 시작하므로
       앞에 빈 칸 열다섯 개를 채워 넣을 수는 없다 — NPC마다 `from`(첫 등장 장)을 두고
       그만큼 빼서 센다. from이 없으면 예전과 똑같이 0장 기준이다. */
    const from = NPCS[id].from || 0;
    const di = clamp(this.chapter - from, 0, DIALOGUE[id].length - 1);
    const lines = DIALOGUE[id][di];
    const choices = [];
  ...
  talkVillager(id) {
    ...
    if (id === 'tamer') {
      ...
      choices.push({ t: '오늘 실은 것을 보자', fn: () => { UI.closeDialogue(); UI.openShop(id); } });
    }
  ...
  tellQuest() {
    ...
    const session = '세션 ' + sessionOf(this.chapter);
  ...
  price(it) {
    const d = idef(it);
    // 값이 고정된 물건(알 등) — 시세도 등급 배수도 타지 않는다
    if (d.fixed) return Math.max(1, Math.round(d.price * (it.c > 1 ? it.c : 1)));
    let base = 12;
    ...
    /* 배수는 딱 요청대로 '지금 값의 몇 배'다 — 기본값(12)을 같이 올리면 4~5배가
       아니라 수십 배가 된다. 기계·설치물 4.5배, 무기 빼고 나머지 1.75배. */
    /* 강판처럼 '놓을 수 있는' 공장 재화는 type이 block이라 기본값이 2였다 — 흙·돌과
       같은 값이 되어 배수를 곱해도 여전히 고물값이다. 공장 재화는 재료 기본값을 밑돈다. */
    if (FACTORY_GOODS.has(it.id)) base = Math.max(base, 12);
    const mul = FACTORY_GOODS.has(it.id) ? FACTORY_PRICE_MUL
      : (PRICE_MUL[d.type] !== undefined ? PRICE_MUL[d.type] : PRICE_MUL_DEFAULT);
    /* 티어 가중 — 뒤로 갈수록 값이 더 벌어진다(priceTierMul, data.js).
       fixed(알)는 위에서 이미 빠져나갔으므로 여기 안 온다. */
    const raw = base * mul * priceTierMulOf(d, it.id) * (it.c > 1 ? it.c : 1)
      * RARITY_MULT[it.r] * this.marketRate(it.id);
  ...
  spawnBoss(id, x, y) {
    /* 배수 1 — 보스는 장을 안 탄다(Enemy 생성자가 boss면 배수를 무시한다).
       여기서 굳이 1을 넘기는 것은 읽는 사람이 "여기 배수가 빠졌나?" 하고
       찾아 헤매지 않게 하려는 것이다. */
    const e = new Enemy(id, x, y, 1);
  ...
  /* 몹의 세기는 **스토리 진행(장)만** 따라간다.
     예전에는 플레이어 레벨도 더했는데(레벨 100이면 +3.0), 레벨을 올릴수록 세상이 같이
     세져서 "강해진 느낌"이 사라졌다. 레벨은 이제 순수하게 내 쪽만 세게 만든다.
     경험치·금화도 같은 배수를 쓰므로, 레벨을 올려도 몹의 보상이 부풀지 않는다.
     장별 상승폭도 0.22 → 0.09로 낮췄다 — 9장에서 2.98배는 "스토리를 미는 것이 곧 벌"이
     될 만큼 가팔랐다. 지금은 9장 1.81배, 마지막 장(17) 2.53배 정도로 완만하다. */
  ...
  saveGame() {
    ...
    try {
      ...
      data.sealed = 1;                                // 이 기록은 서명이 있다는 표시
      const text = JSON.stringify(data);
      localStorage.setItem(slotKey(this.currentSlot), text);
      localStorage.setItem(sigKey(this.currentSlot), saveSign(text));
  ...
  loadGame(slot) {
    ...
    /* 봉인 검사 — 손댄 기록은 열지 않는다. 여는 것을 막을 뿐 지우지는 않는다.
       내 서명 쪽에 문제가 있어 멀쩡한 기록을 잠갔더라도 파일은 그대로 남아 있어야 한다. */
    let head = null;
  ...
  render() {
    ...
    for (let ty = ty0; ty <= ty1; ty++) {
      for (let tx = tx0; tx <= tx1; tx++) {
        ...
        /* 상단 하이라이트는 **하늘에 드러난 윗면**을 흉내 내는 선이다. 예전 조건은
           "윗칸이 고체가 아니면"이라서, 물에 잠긴 바닥이나 잡초·조개가 얹힌 칸에도
           밝은 줄이 그어졌다 — 빛이 닿지 않는 자리에 빛 자국이 남은 셈이다.
           윗칸이 **정확히 공기일 때만** 긋는다. */
    ...
    if (wep) {
      ...
      if (hd && hd.type === 'rod') {
        ...
        if (tool && p.swing <= 0) {
          ...
          const ang = p.swing > 0 ? (p.swingAng + (p.swingDir > 0 ? 1 : -1) * (p.swing / 0.24 - 0.5) * 2.0) : (p.facing > 0 ? -0.4 : Math.PI + 0.4);
          c.rotate(ang);
        }
  ...
  drawEnemy(c, e, sx, sy) {
    ...
    /* 물속 몹은 어둡게 깔린 물 위에 제 색이 묻혀 안 보인다 — 웅덩이 뱀장어(#3a6a5a)는
       ...
       바깥에 테두리를 그리면 판정 박스 모양(네모)이 드러나서 그렇게 하지 않는다. */
    /* 잠겼는지는 **타일로** 본다. 수중 몹(ai swimmer)은 move()에 aquatic으로 들어가
       부력 계산을 건너뛰므로 e.submerged가 늘 0이다 — 그 값을 믿으면 정작 물속 몹만
       테두리를 못 받는다(웅덩이 뱀장어가 그래서 안 보였다). */
    /* 물에 잠긴 몹 전부를 밝히면 물속이 온통 번쩍인다 — 실제로 안 보였던 것은
       몸 색(#3a6a5a)이 어두운 물과 거의 같은 웅덩이 뱀장어 하나뿐이다. 그것만 준다. */
    const wet = e.type === 'grotto_eel' && this.world.liquid(Math.floor(e.cx / TS), Math.floor(e.cy / TS));
    if (this.spritesOn && wet && meta) {
      /* 제 그림을 흰 실루엣으로 만들어 사방 2px 밀어 깔면, **그림 모양 그대로**
         밝은 테두리가 생긴다. 판정 박스에 네모를 두르면 실제 모양과 안 맞아 어색하고,
         제 색을 덧칠하는 것만으로는(어두운 색 + 어두운 색) 물속에서 여전히 안 보인다. */
      c.save();
      c.filter = 'brightness(0) invert(1)';
      c.globalAlpha = 0.5;
      for (const [ox, oy] of [[-2, 0], [2, 0], [0, -2], [0, 2]])
        Sprites.draw(c, e.type, this.enemyFrame(e), sx + ox, sy - dy + oy, e.facing < 0);
      c.restore();
    }
