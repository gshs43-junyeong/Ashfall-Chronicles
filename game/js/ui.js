...
const UI = {
  ...
  refreshQuest() {
    const g = G;
    /* 창 하나에 탭 둘. 업적은 새 패널을 만들지 않고 여기 얹는다 —
       패널을 새로 내면 여는 키를 하나 더 배정해야 하고, "여정의 기록"이라는
       이름이 이미 업적을 담기에 맞는 자리다. */
    ...
    /* 세션 구분은 SESSION_START(data.js) 하나만 본다 — 예전엔 여기 9를 따로 적어 둬서
       세션 3이 늘었을 때 이 창만 세션 2에 멈춰 있었다. */
    const currentSession = 's' + sessionOf(g.chapter);
    const selectedSession = this.questSession || currentSession;
    const TITLES = ['잿빛의 여정', '벽 너머', '가라앉은 쪽'];
    const sessions = SESSION_START.map((start, i) => {
      const end = i + 1 < SESSION_START.length ? SESSION_START[i + 1] : Infinity;
      return {
        key: 's' + (i + 1), label: '세션 ' + (i + 1), title: TITLES[i] || '',
        chapters: CHAPTERS.filter(ch => ch.id >= start && ch.id < end)
      };
    });
  ...
  refreshTownhall() {
    ...
    let h = '';
    // 단계 수는 VILLAGE 표가 정한다 — 3으로 박아 두는 바람에 4단계(여명 교역지)가
    // 표에는 있는데 회관 창에는 안 떴다. 표에 단계를 더하면 여기도 저절로 따라온다.
