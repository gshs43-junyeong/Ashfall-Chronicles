...
/* 이보다 작고 고립된(지상과 안 통하는) 공동은 동굴로 치지 않고 메운다(타일 수).
   ★ 30 쯤으로 낮추면 땅속이 한두 방짜리 굴로 숭숭해진다 — 30 일 때 지표 10칸 아래
...
/* 시설(fac)은 9~10칸, 강화 모루는 11~12칸, 탁자는 14~15칸.
   13칸은 사이 여백이다 — 없으면 재련대·모루가 탁자와 붙어 보인다.
...
const DAWN_OBJ = {
  ...
  // 침대는 2칸 슬롯(44px)을 꽉 채운다 — 그리는 쪽(game.js drawFacility)도 발판을 o.h 바닥에 붙인다
  ...
  // 가구는 다른 물건 옆에서 왜소해 보이지 않게 — 책장은 작업대·용광로와 같은 2칸(44px)
  // (DAWN_INSIDE.shelf 슬롯도 2칸), 탁자는 슬롯을 2칸으로 늘려서 키운다.
...
class World {
  ...
  generate() {
    ...
    // --- 3. 동굴 ---
    /* ★ 동굴은 눈에 띄게 크다 — 노이즈 결이 굵고(0.058, 깊은 곳 0.045) 띠가 넓어서(0.60~0.83)
       굴 하나하나가 길고 넓다. 지표 아래 10칸부터
  ...
  /** 놓기 직전 겹침 검사. 겹치면 콘솔에 어느 둘이 어디서 겹쳤는지 찍는다.
      좌표를 눈대중으로 옮기면 물건이 벽에 박히거나 한자리에 뭉친다 — 그걸 잡는 안전장치다. */
  ...
  upgradeVillage(lv) {
    ...
    if (lv === 2) {
      ...
      for (const b of blocks) {
        ...
        /* 창문 — 1층·2층 양쪽 벽에.
           1층 창은 세로 2칸(gy-4 · gy-5) — 한 칸이면 벽 높이(9~12칸)에 비해 너무 작다. 2층은 한 칸 —
  ...
  buildRuins(rng) {
    ...
    // 여명 마을(dawnCity, x 2850~2960) 지하에 둔다. 다만 여명 마을 바로 아래(x 2905 중심) 심층은 이미 지하 공창(y 210~250)·폭주로(y 306~360)·설계실(폭주로
  ...
  buildCaveZones(rng) {
    ...
    for (let x = 4; x < WW - 4; x++) {
      ...
      for (let y = this.surface[x] + 12; y < HELL_Y - 2; y++) {
        ...
        /* ★ 이끼는 **옆벽에도** 붙인다. 바닥·천장에만 붙이면 벽을 따라 이끼가 끊겨
           "바닥에 깐 카펫"으로 읽힌다. 바닥·옆벽·천장이 한 덩어리로
