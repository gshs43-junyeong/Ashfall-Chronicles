/* ===== engine/assets/image.js — 그림 한 장 불러오기 · 판 번호 · 시트 여백 재기 ===== */
...
export const ASSET_VER = (document.currentScript && document.currentScript.src.split('?')[1]) || '';

/** 그림 하나를 받기 시작한다 — Image 는 곧바로 onImage 로 넘기고(자리부터 잡는다), 끝나면(실패해도) 풀리는 약속을 돌려준다. */
export function imageJob(src, onImage) {
...
export function measurePad(im, frameW, frameH, scale) {
