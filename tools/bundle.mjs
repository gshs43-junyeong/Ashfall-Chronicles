#!/usr/bin/env node
/* src/game/main.ts 에서 import 를 따라 모듈을 묶어 game/js/ashfall.js (+ .map) 하나로 만든다(esbuild · IIFE).
...
if (process.argv[1] && fileURLToPath(import.meta.url) === path.resolve(process.argv[1])) {
  ...
  if (arg === '--check') {
    ...
    if (!same) { console.error('✗ game/js/ashfall.js 가 src/game/ 와 다르다 — node tools/bundle.mjs 로 다시 만들어 함께 커밋할 것'); process.exit(1); }
