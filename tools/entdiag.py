...
"""유적 입구 통로의 발판을 **어디서 왔는지 갈라서** 센다.
  ...
  직접    통로를 팔 때(_carveEntranceShaft) 놓은 것 — foothold 의 수직굴 한 구간뿐이어야 한다
...
오름 = 작은 방이 앞 방보다 위에 놓인 횟수(입구가 무조건 내리막이 아닌지).
  ...
  기준(2026-09, 씨앗 d1~d4 · 입구 28곳): 옛 수직 갱도 3485장 → 비탈 479장
  ...
  if (!window.__origCES) {
    ...
    World.prototype._carveEntranceShaft = function(ex, yTop, yBot, spec, rng){
      ...
      (window.__ent = window.__ent || []).push({ id: spec.id || 'seal',
        kind: spec.entryKind || 'foothold', x0: ex - span - 2, x1: ex + span + 2,
        y0: yTop - 3, y1: yBot, rooms: this._entranceRooms.slice() });
      return r;
    };
  }
}
