...
★ 이 지표만으로는 부족하다 — **tools/animcheck.py 를 같이 봐야 한다.**

  여기서 재는 것은 두 프레임을 **제자리에서** 뺀 값이라, 그림이 통째로 한 칸
  미끄러지기만 해도 30~50% 가 나온다. 관절은 하나도 안 움직이는데 수치는
  멀쩡하다. 실제로 이 값으로 통과시킨 시트 129 칸쌍이 나중에 전부 "평행이동
  뿐" 으로 드러났다. animcheck.py 는 가장 잘 겹쳐지는 자리로 밀어 놓고 남는
  차이를 재므로 그 함정에 안 빠진다.
