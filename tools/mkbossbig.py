#!/usr/bin/env python3
"""5페이즈 보스 넷 — **원본 그림 그대로, 크기만 2배로** 키운다.

  (다섯째인 원형은 여기 없다. 그 그림은 화면 크기 그대로 한 칸씩 손으로
   다시 그렸다 — tools/mkarchetype.py)

■ 두 번 틀리고 나서 정한 것

  1. 절차 도형으로 새로 그렸다 → 그림이 통째로 퇴화했다. 손그림(가시 달린
     공허별 · 리벳 박힌 놋쇠 기계 · 청록 마디가 박힌 궤도 · 용암 금이 간
     바위 입)을 평평한 타원과 네모로 갈아치운 셈이었다.
...
■ 왜 정확히 2배인가
  ...
  환원기의 고리가 팔각형에서 둥근 테로 돌아오는 것이 그 차이다.
...
# ★ 원형(archetype)은 여기서 굽지 않는다. 원본을 늘린 그림이 아니라 화면
#   크기 그대로 192x232 에 한 칸씩 손으로 그린 것이라, tools/mkarchetype.py
#   가 따로 굽는다. 여기 표에 다시 넣으면 그 그림을 덮어쓴다.
...
# 헤파도 원본 세 벌 차이가 3~6% 뿐이다(밝기만 오르고 실루엣이 한 번도 안
