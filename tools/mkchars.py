...
def read_frames():
    ...
    #
    # ★ 지금 이 파일을 그냥 돌리면 안 된다. 원본은 22x41 로 커졌는데 여기 잣대는
    #   아직 20x40 이라, 위 여백을 벗길 때 **머리 맨 윗줄이 통째로 잘려 나간다.**
    #   그리고 다섯 장에는 이미 tools/mkhead.py 로 새 정수리가 그려져 있다 —
    #   여기서 다시 구우면 그 그림이 옛 그림으로 되돌아간다.
    #   다시 쓰려면 FW/FH 를 22/41 로 올리고 부위 분류 좌표를 함께 손봐야 한다.
