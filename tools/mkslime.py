...
■ 프레임 3·4 — 입이 어긋나 있다
  ...
  후보를 넷 구워 놓고 골랐다. 금빛을 살리려 한 셋(테두리 안만 파기 · 입술을
  위아래로 가르기 · 금빛 위에 검은 입 따로)은 전부 입이 아니라 물건으로
  읽혔다. 벌린 입은 **어두운 구멍이 커야** 입으로 읽힌다 — 위를 크게 비우고
  금빛은 아래턱에만 남긴다.
...
def fix_mouth4(px):
    ...
    for y, ch in ((12, 'B'), (13, 'B'), (14, 'D'), (15, 'D'), (16, 'D'),
                  (17, 'D'), (18, 'D'), (19, 'D')):
        for x in range(11, 20):
            px_put(px, 4, x, y, PAL[ch] + (255,))
    # 2. 그늘 · 윗입술 · 벌어진 안 · 아랫입술
    # 후보 넷을 구워 놓고 고른 것이다('다'). 나머지 셋이 왜 떨어졌나:
    #   금빛 테두리 안쪽만 세로로 파기 → 금빛 상자에 난 열쇠구멍
    #   금빛 입술이 위아래로 갈라지고 사이가 검게        → 줄무늬 덩이
    #   금빛은 다문 채 두고 그 위에 검은 입을 따로       → 두 물건
    # 셋 다 '금빛을 살려야 한다'에 매인 탓이다. 벌린 입은 **어두운 구멍이
    # 커야** 입으로 읽힌다 — 금빛은 아래턱에만 남기고 위는 크게 비운다.
    # '.' 은 건드리지 않는 칸.
    rows = ['.FFFF.', 'kkkkkk', 'kkkkkk', 'kkkkkk', '.GGGG.', '..GG..']
    for dy, r in enumerate(rows):
        for i, ch in enumerate(r):
            if ch != '.':
                px_put(px, 4, x0 + i, y0 + dy, PAL[ch] + (255,))
    return 9 * 8 + sum(len(r.replace('.', '')) for r in rows)
