...
def _imp_horn(get, fw, fh):
    ...
    ★ 뿔은 **두 줄**이다 — 밑동 줄 위에 끝 줄(`O#O … O#O`)이 하나 더 얹힌다. 칸1·칸3
      에만 그 끝 줄이 있고 칸0·칸2·칸4 는 밑동만 있어서, 걸을 때(칸2↔칸3) 뿔이
      한 줄씩 자랐다 줄었다 했다. 눈으로 견줘 보면 끝 줄이 있는 쪽이 뾰족한 제 뿔이고
      없는 쪽이 뭉툭하게 잘린 것이라, **없는 칸에 끝 줄을 더해** 일곱 칸을 같게 한다.
      (칸1·칸3 은 이미 그렇게 생겼으므로 아무것도 안 바뀐다.)"""
    HEAD = range(7, 21)
    BASE = ((7, INK), (8, HORN), (9, HORN), (10, INK),
            (13, INK), (14, INK), (18, INK), (19, HORN), (20, INK))
    TIP = ((7, INK), (8, HORN), (9, INK), (19, INK), (20, HORN), (21, INK))
    for y in range(fh):
        if all(get(x, y) for x in HEAD):
            if y < 2:
                return []                    # 자리가 없다 — 덧댈 여백부터 준다
            return ([((x, y - 1), c) for (x, c) in BASE] +
                    [((x, y - 2), c) for (x, c) in TIP])
    return []
...
PATCHES = [
    ...
    # 화염 임프 — 칸마다 **뿔 길이가 달랐다.**
    #
    # 임프는 제자리에서 위아래로 까딱인다. 위로 올라간 칸(0·2·4)은 뿔이 한 줄,
    # 내려간 칸(1·3)은 두 줄이었다. 걸음은 칸2↔칸3 을 오가므로 한 발짝마다 뿔이
    # 자랐다 줄었다 한다. 처음에는 칸2·칸4 에 뿔 **밑동**이 아예 없어서 정수리가
    # 납작했고(머리 윗변이 `##############` 한 줄로 끝났다) 그것부터 되살렸는데,
    # 그러고도 한 줄이 남아 있었다 — 끝 줄이다.
    #
    # 20배로 놓고 견주면 끝 줄이 있는 쪽이 뾰족한 제 뿔이고 없는 쪽이 뭉툭하게
    # 잘린 것이다. 그래서 **짧은 쪽에 맞추지 않고 긴 쪽에 맞춘다** — 일곱 칸 모두
    # 밑동 + 끝 두 줄. 자리는 머리 윗변에서 재므로 여백을 덧대도 따라온다.
    ('imp', 0, _imp_horn, '뿔을 일곱 칸 모두 같은 두 줄짜리로'),
    ('imp', 1, _imp_horn, '뿔을 일곱 칸 모두 같은 두 줄짜리로'),
    ('imp', 2, _imp_horn, '뿔을 일곱 칸 모두 같은 두 줄짜리로'),
    ('imp', 3, _imp_horn, '뿔을 일곱 칸 모두 같은 두 줄짜리로'),
    ('imp', 4, _imp_horn, '뿔을 일곱 칸 모두 같은 두 줄짜리로'),
]
